...
from .photon_module import SPAD_Layer, Photon_Module
...
class DecoderBN(nn.Module):
    def __init__(self, num_features=2048, num_classes=1, bottleneck_features=2048):
        ...
        self.spad_layer = SPAD_Layer([1024,256,128,128])
        self.phomo_1 = Photon_Module(128)
        self.phomo_2 = Photon_Module(128)

    def forward(self, features, spad):
        
        spad_feature = self.spad_layer(spad)
        ...
        x_d4 = self.phomo_1(x_d4,spad_feature[1])
        #         x_d5 = self.up5(x_d4, features[0])
        out = self.conv3(x_d4)
        out = self.phomo_2(out,spad_feature[2])
...
class UnetAdaptiveBins(nn.Module):
    ...
    def forward(self, x,spad, **kwargs):
        unet_out = self.decoder(self.encoder(x), spad,  **kwargs)
