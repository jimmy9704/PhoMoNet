...
from models.spad_simulation import SingleSPADWithLaser, rgb2gray
...
class DataLoadPreprocess(Dataset):
    def __init__(self, args, mode, transform=None, is_for_online_eval=False):
        ...
        self.spad_simulation = SingleSPADWithLaser(args.spad_bins, args.min_depth,
                               args.max_depth, args.laser_fwhm_ps)
        self.signal_count = args.signal_count
        self.sbr = args.sbr
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'train':
            ...
            #spad simulation
            sbr_rand = np.random.randint(5,101)
            intensity = rgb2gray(image)
            counts = self.spad_simulation.simulate(depth=depth_gt.squeeze(2),
                                intensity=intensity,
                                mask=np.ones_like(depth_gt.squeeze(2)),
                                signal_count=self.signal_count,
                                sbr=sbr_rand)
            counts = (counts-counts.min())/(counts.max()-counts.min())

            #viz
            # Image.open(image_path).save("test rgb.png")
            # Image.open(depth_path).save("test depth.png")
            # print("GT",depth_gt.squeeze().shape)
            # plt.imsave('test_gtmagma.png',depth_gt.squeeze()/10, cmap='magma_r')

            #print("plot spad")
            #plt.bar(np.asarray(range(len(counts))),counts)
            #plt.show()

            image, depth_gt = self.random_crop(image, depth_gt, self.args.input_height, self.args.input_width)
            image, depth_gt = self.train_preprocess(image, depth_gt)
            sample = {'image': image, 'depth': depth_gt, 'focal': focal, 'spad':counts}
            ...
            if self.mode == 'online_eval':
                sample = {'image': image, 'depth': depth_gt, 'focal': focal, 'has_valid_depth': has_valid_depth,
                          'image_path': sample_path.split()[0], 'depth_path': sample_path.split()[1], 'spad':counts}
            else:
                sample = {'image': image, 'focal': focal, 'spad':counts}
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        if self.mode == 'train':
            ...
            return {'image': image, 'depth': depth, 'focal': focal, 'spad': spad}
        else:
            has_valid_depth = sample['has_valid_depth']
            return {'image': image, 'depth': depth, 'focal': focal, 'has_valid_depth': has_valid_depth,
                    'image_path': sample['image_path'], 'depth_path': sample['depth_path'], 'spad': spad}
